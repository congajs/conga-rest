...
module.exports = class RestControllerAnnotationHandler {
    ...
    parseRoutesFromFile(container, reader, controller) {
        ...
        definitionAnnotations.forEach((annotation) => {
            ...
            if (annotation instanceof RestUploadAnnotation) {
                this.addUploadToController(container, controllerAnnotation, annotation, controller, controllerObj, routes);
            }
    ...
    /**
     * Set up a controller to handle uploads based on the Rest:Upload annotation
     *
     * @param {Container}                container
     * @param {RestControllerAnnotation} controllerAnnotation
     * @param {RestUploadAnnotation}     annotation
     * @param {Object}                   controller
     * @param {Object}                   controllerObj
     * @param {Array}                    routes
     */
    addUploadToController(container, controllerAnnotation, annotation, controller, controllerObj, routes) {

        if (!RestControllerAnnotationHandler.uploadHandlers) {
            RestControllerAnnotationHandler.uploadHandlers = {};

            const tags = container.getTagsByName('conga.rest.upload.handler');

            tags.forEach((tag) => {
                RestControllerAnnotationHandler.uploadHandlers[tag.getParameter('id')] = container.get(tag.getServiceId());
            });
        }

        if (typeof RestControllerAnnotationHandler.uploadHandlers[annotation.uploadType] === 'undefined') {
            container.get('logger').error('Unknown uploadType "' + annotation.uploadType + '" defined in controller: ' + controller.filePath + ' The valid registered types are: ' + Object.keys(RestControllerAnnotationHandler.uploadHandlers).join(', '));
            process.exit();
        }

        if (annotation.service === null) {
            container.get('logger').error('You are missing the "service" property for the Rest:Upload annotation in controller: ' + controller.filePath);
            process.exit();
        }

        if (!container.has(annotation.service)) {
            container.get('logger').error('The upload service "' + annotation.service + '" does not exist for the controller: ' + controller.filePath + ' Did you remember to add it to your services.yml?')
            process.exit();
        }

        const name = 'conga.rest.' + controllerAnnotation.resource + '.upload';
        let uri = annotation.uriPrefix + controller.prefix;// + '/upload';
        uri = uri.replace('//', '/').replace(/\/+$/, ''); // clean up weird concats

        RestControllerAnnotationHandler.uploadHandlers[annotation.uploadType](
            container,
            name,
            uri,
            controller,
        ...
        //         }
        //
        //         uploader = require('../../../upload/SimpleUploadController');
        //
        //         break;
        //
        //     case 'multipart':
        ...
        //             console.log(container.getParameter('kernel.var_path'));
        //
        //
        //
        //
        ...
        //             req.on('end', function() {
        //
        //                 const parts = multipart.Parse(new Buffer(data,'utf-8'), req.headers['content-type'].split('boundary=')[1]);
        //
        //                 //console.log(data);
        //                 console.log(parts);
        ...
        //
        //
        //         uploader = require('../../../upload/MultipartUploadController');
        //
        //         break;
        // }
        //

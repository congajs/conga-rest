...
RestManager.prototype = {
	...
	serializeListResponse: function(type, data, pagination, includes, req) {

		try {
			return this.serializer.serializeListResponse(type, data, pagination, includes, req);
		} catch (err) {
			this.container.get('logger').error(err.stack);
		}
	...
	createCreatedResponse: function(res, resource) {
		res.return(this.serialize(resource), 201);
	}, 

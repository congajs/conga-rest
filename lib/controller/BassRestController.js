...
module.exports = class BassRestController extends RestController {
	...
	list(req, res) {
		...
		const restManager = this.container.get('rest.manager');
		...
		const sort = this.specification.parseSortFromRequest(req, res);
		const filter = this.speicification.parseFiltersFromRequest(req, res);
		const pagination = this.specification.parsePaginationFromRequest(req, res);

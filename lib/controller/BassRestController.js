...
module.exports = class BassRestController extends RestController {
    ...
    /**
     * Find by query for the current request
     * @param req
     * @param res
     * @param manager
     * @param query
     * @returns {Promise<any>}
     */
    findByQuery(req, res, manager, query) {
        const modelName = this.getModelNameForCurrentResource();

        // TODO: check for nested filters

        // run modifyListQuery to allow the query to be modified
        return this.modifyListQuery(req, res, query).then(() => {

            return manager.findByQuery(modelName, query);
    ...
    list(req, res) {
        ...
        this.findByQuery(req, res, manager, query).then(resources => {
        ...
        }).catch(err => this.sendInternalServerError(res, err));
    }
    ...
    post(req, res) {
        ...
        return this.specification.validateRequest(req, this.resourceType)
            .catch(error => {
                this.sendSpecificationError(error);
            ...
            .then(() => {
                ...
                if (Array.isArray(data)) {
                    ...
                    return this.createMultiple(req, res, data, session, manager)
                        .then(resources => manager.flush().then(() => resources))
                        .then(resources => {

                            return Promise.all(resources.map(resource => {
                                const p = this.onPostCreate(req, res, resource, session, manager);
                ...
                return this.createSingle(req, res, data, session, manager)
                    .then(resource => manager.flush().then(() => resource))
                    .then(resource => {

                        let p = this.onPostCreate(req, res, resource, session, manager);
                        if (!(p instanceof Promise)) {
                            p = Promise.resolve();
                        }
                        return p.then(() => {
            ...
            .catch(err => {
                if (err) {
                    this.container.get('logger').error('[conga-rest] - ' + (err.stack || err));
                    return this.sendInternalServerError(res, err);
                }
            });
    }
    ...
    createSingle(req, res, data, session, manager) {
        ...
        return this.attachRelationshipsToResource(req, res, session, resource, data.relationships, 'POST')

            // validate the model
            .then(() => this.container.get('validator').validate(resource))

            // allow the final resource to be modified or validated before persisting
            .then(() => {
                const p = this.onCreate(req, res, resource, session, manager);
                if (p instanceof Promise) {
                    return p;
                }
                // fail-safe
            ...
            // persist and return the resource
            .then(() => {
            ...
            .catch(err => {
                ...
                this.sendResourceInvalidDataError(res, err);
                return null;
            })
            // catching errors in sendResourceInvalidDataError
                ...
                for (let i in relationships.many) {
                    ...
                    if (relationships.many[i].data.length > 0) {
                        ...
                        promises.push(new Promise((resolve, reject) => {
                            ...
                            const modelName = this.getModelNameForResourceType(
                                relationships.many[i].mapping.relatedType);

                            const meta = manager.registry.getMetadataByName(modelName);
                            const idField = (meta && meta.getIdField()) || {property: 'id'};

                            manager.findWhereIn(modelName, idField.property, ids).then((docs) => {

                                switch (action) {

                                    case 'PATCH':

                                        const existing = resource[i].map(doc => doc.id);
                                ...
                                return Promise.all(docs.map(doc => {
                                    return this.checkRelatedResourcePermissions(
                                        req,
                                        res,
                                        resource,
                                        doc,
                ...
                for (let i in relationships.one) {
                    ...
                    if (relationships.one[i].data !== null) {
                        ...
                        promises.push(new Promise((resolve, reject) => {
                            ...
                            manager.find(
                                this.getModelNameForResourceType(
                                    relationships.one[i].mapping.relatedType),

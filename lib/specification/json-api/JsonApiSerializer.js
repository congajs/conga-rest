...
module.exports = class JsonApiSerializer extends Serializer {
     ...
     * @return {Object}
     */
    serializeData(req, object, sparseFields) {
		...
		if (this.isArray(object)) {
			...
			for (let i = 0, j = object.length; i < j; i++) {
				data.push(this.serializeData(req, object[i], sparseFields));
			}

		} else {
			data = this.serializeObject(req, object, sparseFields);
		}
     ...
     * @return {Object}
     */
	serializeObject(req, object, sparseFields) {
			...
			if (object instanceof Object &&
				...
				for (var idx in object) {
					if (typeof object[idx] !== 'function' &&
						...
						obj[idx] = this.serializeData(req, object[idx], sparseFields);
					}
				}
				...
				if (typeSparseFields == null || (typeSparseFields !== null && typeSparseFields.indexOf(property.attribute)) > -1) {
					if (!property.isSetter && property.expose){
						if (property.targetType === 'method'){
							obj.attributes[property.attribute] = this.serializeData(req, object[property.target]() );
						} else {
							obj.attributes[property.attribute] = this.serializeData(req, object[property.target]);
						}
					}
					...
					if (relationship.type == 'one'){
						...
						if (object[relationship.target] !== null && typeof object[relationship.target] !== 'undefined') {
							obj.relationships[relationship.attribute].data = {
								type: this.getMappingFromObject(object[relationship.target]).type,
						...
						if (typeof object[relationship.attribute] !== 'undefined') {
							object[relationship.attribute].forEach((rel) => {
								obj.relationships[relationship.attribute].data.push({
                                    type: this.getMappingFromObject(rel).type,
									id: rel.id
								});
							});
						}
			...
			if (this.isArray(obj)) {
				...
				for (let i = 0, j = obj.length; i < j; i++) {
					...
					if (cache.indexOf(key) === -1){
						included.push(this.serializeObject(tmp, sparseFields));
						cache.push(key);
					}
				...
				if (obj !== null) {
					...
					if (cache.indexOf(key) === -1){
						included.push(this.serializeObject(obj, sparseFields));
						cache.push(key);
					}

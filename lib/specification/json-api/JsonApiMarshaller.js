    ...
    unmarshalSingle(req, type, group, data) {
        ...
        for (attribute in data.attributes) {
            ...
            if (typeof map !== 'undefined' && map instanceof Object) {
                ...
                const isUpdating = object.id !== null;
                const hasGroupAccess = typeof map.group === 'undefined' || map.group.write.includes(attrGroup);
                const isMapUpdateAllowed = typeof map.update === 'undefined' || map.update === true;
                const isMapWriteAllowed = typeof map.write === 'undefined' || map.write === true;

                if (hasGroupAccess &&
                    (!isUpdating || isMapUpdateAllowed) &&
                    (isUpdating || isMapWriteAllowed)
                ) {
